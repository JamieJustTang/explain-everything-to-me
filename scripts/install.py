...
def main() -> None:
    ...
    if args.host == "gemini":
        ...
        write_new(command, f"---\ndescription: Explain agent work from sivtr memory\n---\n\nRead the current Skill file at `{skill_path}` on every invocation, even if you read it earlier in this conversation; then follow it. This is an explicit user command. Obey any user limit on evidence sources: when the user says only the archive, do not inspect repository files or Git. User request: $@\n")
